...
export function usePianoAudio(): UsePianoAudioReturn {
  ...
  const [waveform, setWaveformState] =
    useState<OscillatorType>(DEFAULT_WAVEFORM);
  const [sustain, setSustainState] = useState(DEFAULT_SUSTAIN);
  const [activeNotes, setActiveNotes] = useState<Map<string, ActiveNote>>(
    new Map()
  );
    ...
    (
      ...
      if (releaseStart) {
        ...
        gainNode.gain.exponentialRampToValueAtTime(
          0.001,
          releaseStart + ADSR_ENVELOPE.release
        ); // Use small value instead of 0 for exponential
      }
    },
    []
  );
  ...
  const playNote = useCallback(
    (note: string, velocity: number = 0.7) => {
      const ctx = audioContextRef.current;
      ...
      if (activeNotesRef.current.has(note)) {
        ...
        if (existingNote) {
          ...
          try {
            ...
            const scheduledGain =
              existingNote.gainNode.gain.getValueAtTime(now);
            ...
            existingNote.gainNode.gain.exponentialRampToValueAtTime(
              0.001,
              now + 0.01
            ); // Quick fade
  ...
  const stopNote = useCallback((note: string) => {
    ...
    try {
      ...
      if (currentGain <= 0.001) {
        ...
        activeNote.gainNode.gain.exponentialRampToValueAtTime(
          0.001,
          now + ADSR_ENVELOPE.release
        );
