...
function Button({
  ...
  const hasOnlyIcon = childrenArray.length === 1 && 
    (React.isValidElement(childrenArray[0]) && childrenArray[0].type === ButtonIcon ||
     (typeof childrenArray[0] === 'string' && childrenArray[0].trim().length <= 2));
...
function ButtonIcon({ children, className }: { children: React.ReactNode; className?: string }) {
